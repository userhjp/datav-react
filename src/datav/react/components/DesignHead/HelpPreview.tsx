    ...
    <div>
      ...
      >
        ...
        <div className="help-container">
          ...
          <ul>
            <li>图层：当前画布所有组件列表。</li>
            <li>组件列表：组件物料列表，点击组件拖拽到画布进行添加组件。</li>
            <li>右侧面板：组件配置面板，对组件属性，数据，交互等配置。</li>
            <li>工具箱：设计器参考线 对齐线 等开关。</li>
            <li>多选：需要时开启，默认关闭，框选完成后后会自动关闭(避免影响到组件正常拖拽)。</li>
          </ul>
          <div className="help-title">常用操作</div>
          <ul>
            <li>右键菜单：组件点击右键弹出操作菜单对组件进行操作。</li>
            <li>画布操作：按住 Space(空格键) 拖拽画布，按住 Ctrl键 并滚动鼠标调整画布大小。</li>
            <li>组件位置：点击组件进行拖拽或选中组件后使用 ↑ → ↓ ← 按键进行微调移动，支持多选同时拖拽或移动。</li>
            <li>组件层级：右键菜单 置顶 置底 上移一层 下移一层 按钮对组件层级进行调整。</li>
