...
export const DataFields: React.FC = observer(() => {
  ...
  const dvData = dataSource.getData(field.form.values.component.id);
  ...
  return (
    <>
      {Object.keys(value?.fields || {}).length > 0 && (
        <ObjectField basePath={field.address} name="fields">
      ...
      <ReadOnlyEditor compId={field.form.values.component.id} />
    </>
  );
});
