...
const Timer: React.FC<IWidgetProps> = ({ options, events }) => {
  const [realDateTime, setRealDateTime] = useState(Date.now());
  const timeRef = useRef(0);
  const emitTimeRef = useRef<NodeJS.Timeout>();
  const timerId = useRef<NodeJS.Timeout>();
  const isEmit = useRef(false);
  const { timing, fixedPoint, textStyle, iconStyle } = options;
  const updateVariables = useDatavEvent(events.changed, null, false);
  const updateVariables2 = useDatavEvent(events.changed2, null, false);

  const iconSty = useMemo<React.CSSProperties>(() => {
    return {
  ...
  const updateDate = () => {
    ...
    if (!timing?.show) {
      updateVariables({ time: timeRef.current });
    }
    setRealDateTime(timeRef.current);
    if (fixedPoint?.show) {
      try {
        const time = Date.parse(fixedPoint.time);
        if (Math.abs(time - timeRef.current) <= 1000 && !isEmit.current) {
          isEmit.current = true;
          updateVariables2({ time: fixedPoint.time });
        }
      } catch (error) {}
    }
  };
  ...
  const emitTime = () => {
    ...
    emitTimeRef.current = setTimeout(() => {
      updateVariables({ time: timeRef.current });
      emitTime();
    }, timing.time);
  };
  ...
  useEffect(() => {
    clearTimeout(emitTimeRef.current);
    if (timing?.show) {
      emitTime();
    }
    return () => clearTimeout(emitTimeRef.current);
  }, [timing]);

  useEffect(() => {
    isEmit.current = false;
